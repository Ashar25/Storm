...
def storm(mode, numsims, numsimyrs, seasons, ptot_scenario,
    ...
    *ET_scaling_factor* is a float that specifies the value of fractional step
    ...
    Duration_pdf_GEV = {'shape': -0.5, 'sigma': 35,
                        'mu': 30}
    try:
    ...
    if mode == 'Validation':
        ...
        Zz = [recs.record[5] for recs in b.shapeRecords()]
        # (X1, Y1) = np.meshgrid(
        ...
        Yin = np.loadtxt(os.path.join(datapath, 'Yin.txt'))
        Xin = np.loadtxt(os.path.join(datapath, 'Xin.txt'))
    ...
    Yyin = np.loadtxt(os.path.join(datapath, 'Yyin.txt'))
    Xxin = np.loadtxt(os.path.join(datapath, 'Xxin.txt'))
    # np.savetxt('Xxin.txt', Xxin)
    # np.savetxt('Yyin.txt', Yyin)
    ...
    print(len(orographic_limits))
    OroGrp1 = np.arange(int(round(min(Zz))), 797+1, 1)
    OroGrp2 = np.arange(798, 877+1, 1)
    OroGrp3 = np.arange(878, int(round(max(Zz))), 1)
    ...
    lambda1 = [84.4*frac for frac in fracs]
    kappa =  [13.8*frac for frac in fracs]
    ...
    for simulations in range(numsims):
        ...
        storm_tot = np.zeros((max_numstorms*numsimyrs, numgauges))
        storm_int = np.zeros((max_numstorms*numsimyrs, numgauges))
        storm_dur = np.zeros((max_numstorms*numsimyrs, numgauges))
        ...
        for syear in range(numsimyrs):
            ...
            for storm in range(max_numstorms):
                ...
                for j in range(numgauges):
                    ...
                    # print(Gauge_matrix[j][master_storm_count,6])

                    # ^interarrival time in hrs
                    Gauge_matrix[j][master_storm_count, 7] = calendar_time
                    storm_tot[master_storm_count, j] =  (
                            rain_int_gauge[j] * duration_val/60)
                    storm_int[master_storm_count, j] =  (
                            rain_int_gauge[j]  )
                    storm_dur[master_storm_count, j] =  (
                        duration_val/60  )
        ...
        Storm_matrix_out_df = pd.DataFrame(Storm_matrix_out,columns=['Storm','Storm Area','Storm Duration','Intensity Duration Curve',
                                                                     'Storm Intensity','Gauges Hit','Recession Value','Storm Total'
            ,'UTM Longitude','UTM Latitude','Simulation Year','Simulation Time'])
        Storm_matrix_out_df.to_excel('D:\IISc\Project\Stormbang\Results\Simulations'
                                     '\{}_sims\{}_year\Storm\StormSim{}Storm_matrix_{}y.xlsx'.format(50,numsimyrs,simulations,numsimyrs))
        # trunc_at1 = np.argmax(np.isclose(storm_tot[0:, numgauges], 0.)) + 1
        # storm_tot_out = storm_tot[:trunc_at1, :]
        stormtot_df = pd.DataFrame(storm_tot)
        stormtot_df = stormtot_df.iloc[0:Storm_matrix_out_df.shape[0],:].copy()
        stormtot_df['Simulation Year'] = Storm_matrix_out_df['Simulation Year'].values
        stormint_df = pd.DataFrame(storm_int)
        stormint_df = stormint_df.iloc[0:Storm_matrix_out_df.shape[0],:].copy()
        stormint_df['Simulation Year'] = Storm_matrix_out_df['Simulation Year'].values
        stormdur_df = pd.DataFrame(storm_dur)
        stormdur_df = stormdur_df.iloc[0:Storm_matrix_out_df.shape[0],:].copy()
        stormdur_df['Simulation Year'] = Storm_matrix_out_df['Simulation Year'].values
        stormtot_df.to_excel('D:\IISc\Project\Stormbang\Results\Simulations\{}_sims'
                                '\{}_year\Gauge\Sim{}GaugeTot_{}y.xlsx'.format(50,numsimyrs,simulations,numsimyrs))
        stormint_df.to_excel('D:\IISc\Project\Stormbang\Results\Simulations\{}_sims'
                                '\{}_year\Gauge\Sim{}Gaugeint_{}y.xlsx'.format(50,numsimyrs,simulations,numsimyrs))
        stormdur_df.to_excel('D:\IISc\Project\Stormbang\Results\Simulations\{}_sims'
                             '\{}_year\Gauge\Sim{}Gaugedur_{}y.xlsx'.format(50,numsimyrs,simulations,numsimyrs))
        # Storm_matrix_out_df = pd.DataFrame(Storm_matrix_out,columns=['Storm','Storm Area','Storm Duration','Intensity Duration Curve',
        #                                                               'Storm Intensity','Gauges Hit','Recession Value','Storm Total'
        #                                                               ,'UTM Longitude','UTM Latitude','Simulation Year','Simulation Time'])
        # Storm_matrix_out_df.to_excel('Sim{}Storm_matrix_{}y.xlsx'.format(simulations,numsimyrs))
...
if __name__ == "__main__":
    ...
    SM, GM, Dur = storm(mode='Simulation', numsims=1, numsimyrs=1, seasons=1,
                        ptot_scenario='ptotC', storminess_scenario='stormsC',
